...
function sysInfo()
{
    $system['name'] = 'skillset';
    $system['version'] = '1.1.0';
    $system['build_version'] = '1.4.3';
...
function strWords($text = null, $length = 10)
{
    return Str::words($text, $length);
}

function strSub($text = null, $length = 10)
{
    return Str::substr($text, $length);
}
...
function getDurationForHumans($number)
{
    return gmdate("H:i", $number * 60);
}

function priceCalculate($number, $discount)
{
...
function calculateAverageRating($averageRating = 0)
{
 
    $fullStars = floor($averageRating);
    $halfStar = ceil($averageRating - $fullStars);
    $emptyStars = 5 - $fullStars - $halfStar;
    $ratingHtml = '<li class="ratings-box">';
    // Full stars
